...
SELECT app_name, price, rating FROM analytics WHERE rating < 3 AND PRICE <> 0 ORDER BY PRICE DESC;
...
-- Find the most out of date app. Hint: You don’t need to do it this way, but it’s possible to do with a subquery: http://www.postgresqltutorial.com/postgresql-max-function/
 SELECT app_name MIN(last_updated) FROM analytics GROUP BY app_name ORDER BY MIN(last_updated);  

-- Find the most expensive app (the query is very similar to #11).
SELECT app_name, MAX(price) FROM analytics GROUP BY app_name ORDER BY MAX(price) DESC;
...
SELECT app_name, reviews, min_installs, min_installs/reviews FROM analytics WHERE min_installs >= 1000000 ORDER BY min_installs/reviews DESC LIMIT 1;